...
public:
	...
	bool gammarCorrection;

	// constructor, expects a filepath to a 3D model.
	Model(std::string const& path, bool gamma = false) : gammarCorrection(gamma)
	{
		loadModel(path);
	}
...
private:
	...
	void processNode(aiNode* node, const aiScene* scene)
	{
		...
		// after we've processed all of the meshes (if any) we than recursively process each of the chidren nodes
	...
	Mesh processMesh(aiMesh* mesh, const aiScene* scene)
	{
		...
		for (unsigned int i = 0; i < mesh->mNumVertices; i++)
		{
			...
			// texture coordinates
			if (mesh->mTextureCoords[0]) // check mesh contain texture coordinates
			{
				glm::vec2 vec;
